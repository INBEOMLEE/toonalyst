...
<body>
	<div class="freeboard">
		...
		<div class="array_list">
			...
			<c:forEach items="${map.myBoardList }" var="blist">
				<span class="array_style">
				
				<a href="${path }/board/myboard?bcategory=${blist.BCATEGORY}" <c:out value="${blist.BCATEGORY == map.bcategory ? 'class=order':''}"/>>${blist.BOARDNAME } (${blist.COUNT })</a>
				
				<c:if test="${blist.BCATEGORY == map.bcategory}">
					<c:set var="boardname" value="${blist.BOARDNAME }"/>
				</c:if>
				</span>
			</c:forEach>
		</div>
				...
				<div class="pagination_box">
					<div class="pagination">
						<c:if test="${map.pager.curBlock > 1}">
							<a href="${path}/board/myboard?curPage=1&keyword=${map.keyword}&search_option=${map.search_option}&bcategory=${map.bcategory}"><i class="fas fa-angle-double-left"></i></a>
							<a href="${path}/board/myboard?curPage=${map.pager.blockBegin - 10}&keyword=${map.keyword}&search_option=${map.search_option}&bcategory=${map.bcategory}"><i class="fas fa-angle-left"></i></a> 
						</c:if>
						
						<c:forEach begin="${map.pager.blockBegin}" end="${map.pager.blockEnd}" var="idx">
							<a href="${path}/board/myboard?curPage=${idx}&keyword=${map.keyword}&search_option=${map.search_option}&bcategory=${map.bcategory}" <c:out value="${map.pager.curPage == idx ? 'class=active':''}"/>>${idx}</a>
						</c:forEach>
						
						<c:if test="${map.pager.curBlock < map.pager.totBlock}">
							<a href="${path}/board/myboard?curPage=${map.pager.blockEnd + 1}&keyword=${map.keyword}&search_option=${map.search_option}&bcategory=${map.bcategory}"><i class="fas fa-angle-right"></i></a>
							<a href="${path}/board/myboard?curPage=${map.pager.totPage}&keyword=${map.keyword}&search_option=${map.search_option}&bcategory=${map.bcategory }"><i class="fas fa-angle-double-right"></i></a> 
						</c:if>
					</div>
				</div>
				<!-- 검색창 부분 -->
				<div class="board_search">
					<div class="search_wrap">
						<select name="search_option" class="search_option">
							<option value="myall" selected="selected">제목+내용</option>
...
$(document).ready(function(){
	...
	function search_validation(){
		...
		location.href="${path}/board/myboard?sort_option="+sort_option+"&search_option="+search_option+"&keyword="+keyword+"&bcategory="+bcategory;
	}
});
